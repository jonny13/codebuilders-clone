...
def standardize(quote):
  return (''.join(e for e in quote if e.isalnum()))
...
def genRandRail(num, quote, enc):
    ...
    if enc == "D":
        ...
        x["railOffset"] = offset
        x["question"] = (
            "<p>Decode this sentence with the Railfence cipher with "
            + str(railNum)
            + " rails and an offset of "
            + str(offset)
            + ".</p>"
        )
        x["points"] = "250"
    if enc == "C":
        x["operation"] = "crypt"
        x["variableRails"] = True
        x["railOffset"] = offset
        x["question"] = (
            "<p>Decode this sentence with the Railfence cipher with an unkown number of rails and unknown offset. The first five letters are "
            + standardize(quote)[:5]
            + ". </p>"
        )
        x["points"] = "300"
...
def getRandWord(min, max):
    f = open("codebuilder-main/words.txt", "r")
...
def genQuotes(n):
    quotes = open("codebuilder-main/quotes.txt", "r")
    l = []
...
def genSpanishQuote(min, max):
    json_file = open("codebuilder-main/spanish.json", "r")
    l = []
...
def get2x2Key():
    f = open("codebuilder-main/2x2hillwords.txt", "r")
...
def get3x3Key():
    f = open("codebuilder-main/3x3hillwords.txt", "r")
...
def genTest(na, preset):
    ...
    file = open("codebuilder-main/CodeTests/" + na + ".json", "w+")
    file.write(json.dumps(test))
    file.close()
    return json.dumps(test)


TOKEN = open("codebuilder-main/token.txt", "r").read()
...
with open("codebuilder-main/admin.txt") as f:
    a = f.readlines()
admin_list = [str(x.strip()) for x in a]
print(admin_list)

with open("codebuilder-main/secret.txt") as f:
    a = f.readlines()
secret_list = [str(x.strip()) for x in a]
print(secret_list)

with open("codebuilder-main/cancel.txt") as f:
    a = f.readlines()
...
async def gen(ctx, name, pre):
    try:
        if "." not in name and "/" not in name and name != "spanish":
            ...
            with open("codebuilder-main/CodeTests/" + name + ".json", "r") as f:
...
keep_alive()
bot.run(TOKEN)
qupote="HELLO. THis iS a TEST."
def standardizeQuote(qupote):
  newQuote = ''.join(e for e in qupote if e.isalnum())
  return newQuote